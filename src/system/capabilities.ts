...
const definitions: CapabilityDescriptor[] = [
  ...
  { id: 'entity.wallet.balance.set', description: 'Set one existing canonical wallet balance for one resolved entity.', accepted_inputs: { entity: 'entity', amount: 'number', currency: 'string?' }, output: 'canonical wallet', access: 'write', side_effect: 'canonical-state', affected_surfaces: ['entities.wallet.balances'], permissions: ['write canonical state'], dependencies: ['entity.lookup'], provider_requirements: ['economy.wallet'], version: '1.0.0', retry: 'idempotency-key', idempotency: 'request receipt and expected revision', implemented: true },
