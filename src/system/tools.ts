...
export const tools: ToolDescriptor[] = [
  ...
  { tool_id: 'behavior.configure', name: '设置叙述与对话风格', description: '把玩家的风格要求保存为受约束的运行时配置（旁白、对话或助手）。', category: 'BEHAVIOR_CONFIGURATION',
    input_schema: { scope: 'narration|dialogue|assistant', instruction: 'string' }, output_schema: 'updated behaviour config', required_capabilities: [], permissions: ['framework configuration'], side_effect_level: 'configuration', confirmation_policy: 'ambiguous-only', supported_platforms: ['desktop','mobile'] },
  { tool_id: 'behavior.clear', name: '恢复默认风格', description: '清除一个范围或全部运行时风格配置。', category: 'BEHAVIOR_CONFIGURATION',
    input_schema: { scope: 'narration|dialogue|assistant?' }, output_schema: 'updated behaviour config', required_capabilities: [], permissions: ['framework configuration'], side_effect_level: 'configuration', confirmation_policy: 'never', supported_platforms: ['desktop','mobile'] },
